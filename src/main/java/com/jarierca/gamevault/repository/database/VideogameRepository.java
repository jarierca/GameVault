...
public class VideogameRepository implements PanacheRepository<Videogame> {
	...
	public List<VideogameDTO> findRandomGames(int limit) {
		List<Videogame> randomGames = find("ORDER BY RANDOM()").page(Page.of(0, limit)).list();

		return randomGames.stream().map(videogame -> {
			...
			return new VideogameDTO(videogame.getId(), videogame.getTitle(), videogame.getReleaseDate(), imageDTOs);
		}).collect(Collectors.toList());
	}
